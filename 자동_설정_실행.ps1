...
try {
    aws eks update-kubeconfig --name $CLUSTER_NAME --region $AWS_REGION 2>&1 | Out-Null
    Write-Host "✓ kubeconfig 설정 완료" -ForegroundColor Green
} catch {
    Write-Host "✗ kubeconfig 설정 실패 (권한 문제일 수 있음)" -ForegroundColor Red
    Write-Host "  이후 단계는 계속 진행됩니다..." -ForegroundColor Yellow
}
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✓ 클러스터 연결 성공" -ForegroundColor Green
        kubectl get nodes
    } else {
        Write-Host "✗ 클러스터 연결 실패" -ForegroundColor Red
        Write-Host "  GitHub Actions는 별도 자격 증명을 사용하므로 계속 진행합니다..." -ForegroundColor Yellow
    }
} catch {
    Write-Host "✗ kubectl 명령 실패" -ForegroundColor Red
}
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✓ Namespace '$NAMESPACE'가 이미 존재합니다" -ForegroundColor Green
    } else {
        kubectl create namespace $NAMESPACE 2>&1 | Out-Null
        if ($LASTEXITCODE -eq 0) {
            Write-Host "✓ Namespace '$NAMESPACE' 생성 완료" -ForegroundColor Green
        } else {
            Write-Host "✗ Namespace 생성 실패" -ForegroundColor Red
        }
    }
} catch {
    Write-Host "✗ Namespace 작업 실패" -ForegroundColor Red
}
...
foreach ($repo in $repositories) {
    try {
        aws ecr describe-repositories --repository-names $repo --region $AWS_REGION 2>&1 | Out-Null
        if ($LASTEXITCODE -eq 0) {
            Write-Host "✓ ECR 리포지토리 '$repo' 존재 확인" -ForegroundColor Green
        } else {
            Write-Host "✗ ECR 리포지토리 '$repo' 없음 (권한 문제 또는 미생성)" -ForegroundColor Yellow
        }
    } catch {
        Write-Host "✗ ECR 확인 실패: $repo" -ForegroundColor Red
    }
}
...
Write-Host "=====================================================================" -ForegroundColor Cyan
Write-Host "   명령어 복사용" -ForegroundColor Cyan
Write-Host "=====================================================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "# GitHub Secrets 링크 열기 (브라우저에서):" -ForegroundColor Yellow
Write-Host "https://github.com/$REPO_OWNER/$REPO_NAME/settings/secrets/actions" -ForegroundColor Cyan
Write-Host ""
Write-Host "# Git 커밋 및 푸시:" -ForegroundColor Yellow
Write-Host "git add ." -ForegroundColor Cyan
Write-Host "git commit -m `"chore: Kubernetes CI/CD 설정 완료`"" -ForegroundColor Cyan
Write-Host "git push origin main" -ForegroundColor Cyan
Write-Host ""
...
## 설정 후:
1. git add .
2. git commit -m "chore: Kubernetes CI/CD 설정 완료"
3. git push origin main
"@

$secretsContent | Out-File -FilePath "GITHUB_SECRETS_설정.txt" -Encoding UTF8

Write-Host "✓ 설정 정보가 'GITHUB_SECRETS_설정.txt' 파일에 저장되었습니다" -ForegroundColor Green
Write-Host ""

# 브라우저로 GitHub Secrets 페이지 열기
$openBrowser = Read-Host "GitHub Secrets 페이지를 브라우저로 열까요? (y/n)"
if ($openBrowser -eq "y" -or $openBrowser -eq "Y") {
    Start-Process "https://github.com/$REPO_OWNER/$REPO_NAME/settings/secrets/actions"
    Write-Host "✓ 브라우저에서 GitHub Secrets 페이지를 열었습니다" -ForegroundColor Green
}
